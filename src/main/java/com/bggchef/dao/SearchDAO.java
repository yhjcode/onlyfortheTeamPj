	...
	public List<RecipeDTO> searchRecipe(String keyword, int currentPage, int pageSize) {
		...
		try {
			...
			String sql = "SELECT * FROM (SELECT ROWNUM AS RNUM, A.* FROM ("
					+ "SELECT R.RECIPE_ID, R.USER_ID, R.CATEGORY_ID, R.TITLE, R.THUMBNAIL, R.DESCRIPTION, "
					+ "R.SERVINGS, R.COOK_TIME, R.DIFFICULTY, R.VIEW_COUNT, R.IS_DELETED, R.CREATED_AT, R.LINK, "
					+ "U.NICKNAME, NVL(ar.AVG_RATING, 0) AS AVG_RATING "
					...
					+ "WHERE UPPER(R.TITLE) LIKE UPPER(?) AND R.IS_DELETED = 0 "
	...
	public List<UserDTO> searchUser(String keyword, int currentPage, int pageSize) {
		...
		try {
			...
			String sql = "SELECT * FROM (SELECT ROWNUM AS RNUM, A.* FROM ("
					...
					+ "AVG(ar.AVG_RATING) AS AVGAVG_RATING "
					+ "FROM USERS U LEFT JOIN RECIPE R ON U.USER_ID = R.USER_ID "
					+ "LEFT JOIN (SELECT RECIPE_ID, AVG(RATING) AS AVG_RATING FROM REVIEW "
					+ "           WHERE IS_DELETED = 0 AND RATING IS NOT NULL GROUP BY RECIPE_ID) ar "
					+ "ON R.RECIPE_ID = ar.RECIPE_ID "
					+ "WHERE U.NICKNAME LIKE ? AND U.IS_DELETED = 0 "
